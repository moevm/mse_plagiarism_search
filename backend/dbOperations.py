...
def upload_file():
	if request.method == 'POST':
		...
		if file and (allowed_file(file.filename)
					 or allowed_archive(file.filename)):
			...
			if ".zip" in filename:
				...
				addOneFile(app.config['UPLOAD_FOLDER'],
						   filename)  #TODO: Удалить файл после всех операций?
				return jsonify({
					"ok": "ok"
...
def shorterString(string):
	...
	for i in range(0, (len(string) // 253) + 1):
		if i > 0:
			...
			result.append(string[0 + (i * 255):min(255 *
												   (i + 1), len(string))])
	for v in result:
		print(len(v))
	return result
...
def getEntryFiles(id):
	q = Query.from_(db.tables["File"]).select("path").where(
		db.tables["File"].entryId == int(id))
	return jsonify(executeQ(q, True))


@app.route('/getFile/<id>', methods=['GET'])
def getFile(id):
	q = Query.from_(db.tables["CodeFragment"]).select("text").where(
		db.tables["CodeFragment"].fileId == int(id)).orderby('order',
															 order=Order.asc)
...
def renameEntry(id):
	#print(request.args)
	q = Query.update(db.tables["Entry"]).set(
		db.tables["Entry"].name,
...
def deleteEntry(id):
	...
	q = Query.from_(
		db.tables["Entry"]).delete().where(db.tables["Entry"].id == int(id))
	executeQ(q)
	return jsonify({"ok": "ok"})
